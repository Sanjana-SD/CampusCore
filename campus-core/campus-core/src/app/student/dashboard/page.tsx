...
export default function StudentDashboard() {
  ...
  const fetchStudentData = async () => {
    try {
      ...
      // Load static data
      const attRes = await fetch(`/api/admin/crud?table=attendance&filterCol=student_id&filterVal=${user.id}`);
      setAttendance(await attRes.json());

      const asRes = await fetch('/api/admin/crud?table=assignments');
      setAssignments(await asRes.json());

      const sbmsRes = await fetch(`/api/admin/crud?table=submissions&filterCol=student_id&filterVal=${user.id}`);
      setSubmissions(await sbmsRes.json());

      const mkRes = await fetch(`/api/admin/crud?table=marks&filterCol=student_id&filterVal=${user.id}`);
      setMarks(await mkRes.json());

      const jbRes = await fetch('/api/admin/crud?table=jobs');
      setJobs(await jbRes.json());

      const apRes = await fetch(`/api/admin/crud?table=applications&filterCol=student_id&filterVal=${user.id}`);
      ...
      setChatRoster(ctData.filter((c: any) => c.id !== user.id));
